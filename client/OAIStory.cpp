...
OAIChat OAIStory::getChat() const {
    return m_chat;
}
void OAIStory::setChat(const OAIChat &chat) {
    m_chat = chat;
    m_chat_isSet = true;
}

bool OAIStory::is_chat_Set() const{
    return m_chat_isSet;
}

bool OAIStory::is_chat_Valid() const{
    return m_chat_isValid;
}

qint32 OAIStory::getId() const {
    return m_id;
}
void OAIStory::setId(const qint32 &id) {
    m_id = id;
    m_id_isSet = true;
}
...
bool OAIStory::isValid() const {
    ...
    return m_chat_isValid && m_id_isValid && true;
}
